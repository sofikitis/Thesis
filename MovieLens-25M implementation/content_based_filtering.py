...
def create_user_profile(user_profile, movie_profiles):
    final_profile = np.zeros(len(movie_profiles[0].tags))

    for i in range(0, len(user_profile.movies)):
        movie_id = user_profile.movies[i]
        movie_profile = np.zeros(len(movie_profiles[0].tags))
        for movie in movie_profiles:
            if movie.ide == movie_id:
                movie_profile = movie.tags
                break
        final_profile = final_profile + np.multiply(user_profile.ratings[i], movie_profile)

    final_profile = [(x - np.min(final_profile)) / (np.max(final_profile) - np.min(final_profile))
                     for x in final_profile]

    return final_profile


# Function to find the most relevant movies to use for the content based filtering
def find_relevant_movies(target_movie, all_movies, retrieval_parameter, n_of_movies):
    scores = calculate_relevance_scores(target_movie, all_movies, retrieval_parameter)

    relevant_movies_id = scores[0:n_of_movies]
    relevant_movies = []

    # Create a list with the topk (relevant) movies
    for cur_id in relevant_movies_id:
        relevant_movies.append(all_movies[cur_id[1], :])

    return np.array(relevant_movies)


def run_content_based_filtering(user_ratings_profile, profiles_matrix, high_rated_movies):
    ...
    retrieval_parameter = np.ones(len(target_user))

    """
    After creating the user-tags_profile we don't care about the 
    movie ids so we create a matrix with only the tag relevance
    """
    tag_relevance_matrix = []
    for movie in high_rated_movies:
        tag_relevance_matrix.append(movie.tags)

    tag_relevance_matrix = np.array(tag_relevance_matrix)

    # Use the top 1.000 relevant for the content based filtering to save time
    tag_relevance_matrix = find_relevant_movies(target_user, tag_relevance_matrix, retrieval_parameter, 2000)

    # ConvQ
    topk_movies_ConvQ = convergenceQ(target_movie=target_user, all_movies=tag_relevance_matrix,
                                     retrieval_parameter=retrieval_parameter)

    # ConvM
    topk_movies_ConvM = convergenceM(target_movie=target_user, all_movies=tag_relevance_matrix,
                                     retrieval_parameter=retrieval_parameter)

    # Initialize game
    topk_movies_game = game(target_movie=target_user, all_movies=tag_relevance_matrix,
                            retrieval_parameter=retrieval_parameter)
