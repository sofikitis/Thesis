...
def fitness(ind):
    fit = wave(ind, TARGET)     # * stats.spearmanr(ind, TARGET)[0]
    return fit
...
def make_next_generation(previous_population):
    # elitism
    next_generation = [previous_population[0], previous_population[1]]
...
def run_ga(target, all_chromosomes, id_of_movies):
    global TARGET
    TARGET = target
    count = 0
    ...
    for i in range(GENERATIONS):
        ...
        if new_fitness_sum <= old_fitness_sum:
            ...
            if count >= GEN_WITHOUT_IMPROVEMENT:
                break

    old_generation = np.array(old_generation)
    avg_of_movies = [col[np.nonzero(col)].mean() for col in old_generation.transpose()]

    predicted_ratings = [[r, ide] for r, ide in zip(avg_of_movies, id_of_movies)]
